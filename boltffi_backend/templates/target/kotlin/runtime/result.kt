{% if split_data_package %}internal{% else %}private{% endif %} fun <T, E> WireReader.readResult(
    readOk: (WireReader) -> T,
...
{% if split_data_package %}internal{% else %}private{% endif %} fun <T, E> WireWriter.writeResult(
...
{% if split_data_package %}internal{% else %}private{% endif %} inline fun <T, E, R> BoltFFIResult<T, E>.fold(ok: (T) -> R, err: (E) -> R): R =
    when (this) {
        is BoltFFIResult.Ok -> ok(value)
        is BoltFFIResult.Err -> err(error)
    }

{% if split_data_package %}internal{% else %}private{% endif %} inline fun <T, E> BoltFFIResult<T, E>.wireSize(
    okSize: (T) -> Int,
    errSize: (E) -> Int,
): Int =
    1 + when (this) {
